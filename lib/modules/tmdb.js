...
export default async function tmdb ({ meta, getLog, opt }) {
  const { debug } = getLog('tmdb')
  ...
  if (meta.imdbId) {
    debug('lookup by imdb id')
    meta.tmdb = await imdbId(meta.imdbId, apiKey)
      .catch(e => { throw e })
    return
  }
  if (!meta.parsedName) throw new Error('no imdb id and no parsed name')
  if (meta.parsedName.mediaClass === 'tv') {
    debug('lookup tv ep by parsed name')
    meta.tmdb = await tv(meta.parsedName, apiKey)
      .catch(e => { throw e })
    return
  }
  if (meta.parsedName.mediaClass === 'movie') {
    debug('lookup movie by parsed name')
    meta.tmdb = await movie(meta.parsedName, apiKey)
      .catch(e => { throw e })
    return
  }
  throw new Error(`tmdb, no id and bad type: ${meta.parsedName.mediaClass}`)
}
